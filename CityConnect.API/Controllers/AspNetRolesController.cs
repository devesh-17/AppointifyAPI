...
namespace CityConnect.API.Controllers
{
    ...
    {
        private readonly IHtmlLocalizer<AspNetRolesController> _localizer;
        private readonly IAspNetRolesService _AspNetRolesService;       
       
        public AspNetRolesController(IAspNetRolesService AspNetRolesService, IHtmlLocalizer<AspNetRolesController> localizer)
        {
        ...
        public async Task<object> GetAll()
        {
            ...
            {
                var result = (await _AspNetRolesService.GetAllAsync());
        ...
        public async Task<object> Get(int id)
        {
            return await GetDataWithMessage(async () =>
            {
                var result = await _AspNetRolesService.GetAsync(id);
                return Response(result, string.Empty);
            });
        }

        [HttpPost]
        public async Task<object> Post([FromBody] AspNetRoles model)
        {
            return await GetDataWithMessage(async () =>
            {
        ...
        private async Task<Tuple<AspNetRoles, string, DropMessageType>> AddAsync(AspNetRoles model)
        {
            var flag = await _AspNetRolesService.AddAsync(model);
            if (flag)
            {
        ...
        private async Task<Tuple<AspNetRoles, string, DropMessageType>> UpdateAsync(AspNetRoles model)
        {
            var flag = await _AspNetRolesService.UpdateAsync(model);
            if (flag)
        ...
        public async Task<object> Delete(int id)
        {
            return await GetDataWithMessage(async () =>
            {
                var flag = await _AspNetRolesService.DeleteAsync(id);
                if (flag)
