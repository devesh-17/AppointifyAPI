...
namespace CityConnect.API.Controllers
{
    ...
    public class AspNetUserClaimsController : BaseApiController
    {
        private readonly IHtmlLocalizer<AspNetUserClaimsController> _localizer;
        private readonly IAspNetUserClaimsService _AspNetUserClaimsService;       
       
        public AspNetUserClaimsController(IAspNetUserClaimsService AspNetUserClaimsService, IHtmlLocalizer<AspNetUserClaimsController> localizer)
        {
            _AspNetUserClaimsService = AspNetUserClaimsService;            
        ...
        public async Task<object> GetAll()
        {
            ...
            {
                var result = (await _AspNetUserClaimsService.GetAllAsync());
        ...
        public async Task<object> Get(int id)
        {
            return await GetDataWithMessage(async () =>
            {
                var result = await _AspNetUserClaimsService.GetAsync(id);
                return Response(result, string.Empty);
            });
        }

        [HttpPost]
        public async Task<object> Post([FromBody] AspNetUserClaims model)
        {
            return await GetDataWithMessage(async () =>
            {
        ...
        private async Task<Tuple<AspNetUserClaims, string, DropMessageType>> AddAsync(AspNetUserClaims model)
        {
            var flag = await _AspNetUserClaimsService.AddAsync(model);
            if (flag)
            {
        ...
        private async Task<Tuple<AspNetUserClaims, string, DropMessageType>> UpdateAsync(AspNetUserClaims model)
        {
            var flag = await _AspNetUserClaimsService.UpdateAsync(model);
            if (flag)
        ...
        public async Task<object> Delete(int id)
        {
            return await GetDataWithMessage(async () =>
            {
                var flag = await _AspNetUserClaimsService.DeleteAsync(id);
                if (flag)
