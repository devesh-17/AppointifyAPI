...
using CityConnect.Migrations.DbMigrations;
using AAT.API.Filters;
using CityConnect.Domain.Models;

namespace CityConnect.API
{
    public class Startup
    {
        ...
        {
            services.AddMvc();
            services.AddDbContext<CityConnectContext>(options => options.UseLazyLoadingProxies(false).UseSqlServer(Configuration.GetConnectionString("DefaultConnection")));
            services.AddIdentity<ApplicationUser, IdentityRole>().AddEntityFrameworkStores<CityConnectContext>().AddDefaultTokenProviders();
